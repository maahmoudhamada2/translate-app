...
export default function LangSelectionList({ isInput, toggleList }) {
  const [langList, setLangList] = useState(languagesList);
  ...
  const handleLangClick = (e) => {
    ...
    updateChoosedLang(whichBox, "name", e.target.labels[0].textContent);
    updateChoosedLang(whichBox, "code", inputValue);
    updateChoosedLang(whichBox, "checked", true);
    updateLangPairs(whichBox, inputValue);
    toggleList();
  };

  const handleSearch = (e) => {
    const regPat = `^${e.target.value}`;
    const reg = new RegExp(regPat, "i");
    setLangList(() => languagesList.filter((lang) => lang.name.match(reg)));
  };
